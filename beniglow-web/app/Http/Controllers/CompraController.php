...
class CompraController extends Controller
{
    public function index(Request $request)
    {
        ...
        $resumenCompras = (clone $query)->get(['total', 'estado']);
        $resumen = [
            'cantidad' => $resumenCompras->count(),
            'total' => $resumenCompras->sum('total'),
            'recibidas' => $resumenCompras->where('estado', 'recibida')->count(),
        ];

        $compras = $query->orderByDesc('fecha_compra')->paginate(15);
        $proveedores = Proveedor::where('activo', true)->orderBy('razon_social')->get();
        return view('compras.index', compact('compras', 'proveedores', 'resumen'));
    }
