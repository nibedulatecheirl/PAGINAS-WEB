...
const apiBaseUrl = (() => {
  ...
  const isLocalStatic =
    ...
    ((host === "127.0.0.1" || host === "localhost") && window.location.port !== "8000");
...
function mapProduct(product) {
  const details = [
    product.linea,
    product.tipo_piel ? `Piel: ${product.tipo_piel}` : null,
    product.acabado ? `Acabado: ${product.acabado}` : null,
    product.presentacion || product.volumen,
  ].filter(Boolean);
...
async function loadCatalog() {
  const [productsResponse, categoriesResponse, promosResponse] = await Promise.all([
    fetchJson("/catalogo/productos?per_page=100&con_stock=1"),
    fetchJson("/catalogo/categorias"),
    fetchJson("/catalogo/promociones"),
  ]);
...
window.BENI = {
  ...
  whatsapp: {
    ...
    link: waLink,
    general: waLink(
      "Hola, vengo de la web de BeniGlow Store. Quisiera información sobre productos de skincare."
    ),
    productos: waLink(
      "Hola, vengo de la web de BeniGlow Store. Quisiera consultar productos de skincare."
    ),
    rutinas: waLink(
      "Hola, quiero armar una rutina skincare con BeniGlow Store."
    ),
    promos: waLink(
      "Hola, quiero consultar las promociones de BeniGlow Store."
    ),
  },
  ...
  brand: {
    ...
    facebook:
      "https://www.facebook.com/profile.php?id=61585059257252&locale=es_LA",
  ...
  routines: [
    {
      ...
      message:
        "Hola, quiero armar una rutina básica de día con BeniGlow Store.",
    },
    {
      id: "glow",
      title: "Rutina glow",
    ...
    },
    {
      ...
      message:
        "Hola, quiero armar una rutina para piel sensible con BeniGlow Store.",
    },
  ],

  benefits: [
    { icon: "ShieldCheck", title: "Productos originales", text: "Seleccionados con cuidado para tu piel." },
    { icon: "Plane", title: "Traídos desde YesStyle", text: "Marcas coreanas reconocidas en cada pedido." },
    { icon: "MessageCircle", title: "Asesoría personalizada", text: "Te orientamos según tu tipo de piel." },
    { icon: "Truck", title: "Envíos a todo el Perú", text: "Despachos seguros desde Tacna." },
    { icon: "Store", title: "Entrega en tienda o punto", text: "Coordinamos la entrega contigo." },
    { icon: "Clock", title: "Atención 24 horas", text: "Te respondemos lo antes posible." },
    { icon: "Wallet", title: "Yape, Plin, transferencia y POS", text: "Paga como te quede mejor." },
    { icon: "Sparkles", title: "Recomendaciones para tu piel", text: "Rutinas según tu necesidad." },
  ],

  steps: [
    { n: "01", title: "Elige tus productos", text: "Revisa el catálogo y agrega tus favoritos al pedido." },
    { n: "02", title: "Registra tu pedido", text: "Deja tus datos y coordinamos contigo la confirmación." },
    { n: "03", title: "Confirmamos stock", text: "Validamos disponibilidad y forma de entrega." },
    { n: "04", title: "Realiza el pago", text: "Yape, Plin, transferencia, efectivo o POS." },
    { n: "05", title: "Recibe tu pedido", text: "Envío a todo el Perú o entrega en tienda." },
  ],
