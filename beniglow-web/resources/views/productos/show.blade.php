        ...
        <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="grid grid-cols-2 gap-3 mt-4">
                <div class="bg-emerald-50 rounded-lg p-3 text-center">
                    <p class="text-xs text-emerald-600 font-semibold">PRECIO WEB</p>
                    <p class="text-2xl font-bold text-emerald-700">{{ $moneda }}{{ number_format($producto->precio_final_web, 2) }}</p>
                </div>
    ...
    <div class="lg:col-span-2 space-y-5">
        <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div><p class="text-slate-500">Visible web</p><p class="font-semibold">{{ $producto->visible_web ? 'Si' : 'No' }}</p></div>
                <div><p class="text-slate-500">Destacado web</p><p class="font-semibold">{{ $producto->destacado_web ? 'Si' : 'No' }}</p></div>
                <div><p class="text-slate-500">Slug web</p><p class="font-semibold font-mono">{{ $producto->slug ?: '-' }}</p></div>
                <div><p class="text-slate-500">Marca</p><p class="font-semibold">{{ $producto->marca ?: '-' }}</p></div>
                <div><p class="text-slate-500">Linea</p><p class="font-semibold">{{ $producto->linea ?: '-' }}</p></div>
                <div><p class="text-slate-500">Tono</p><p class="font-semibold">{{ $producto->tono ?: '-' }}</p></div>
                <div><p class="text-slate-500">Presentacion</p><p class="font-semibold">{{ $producto->presentacion ?: '-' }}</p></div>
                <div><p class="text-slate-500">Tipo de piel</p><p class="font-semibold">{{ $producto->tipo_piel ?: '-' }}</p></div>
                <div><p class="text-slate-500">Acabado</p><p class="font-semibold">{{ $producto->acabado ?: '-' }}</p></div>
                <div><p class="text-slate-500">Volumen</p><p class="font-semibold">{{ $producto->volumen ?: '-' }}</p></div>
            ...
            @if($producto->ingredientes_clave)
                <div class="mt-4 pt-4 border-t border-slate-100">
                    <p class="text-slate-500 text-sm mb-1">Ingredientes clave</p>
                    <p class="text-slate-700">{{ $producto->ingredientes_clave }}</p>
                </div>
            @endif
        </div>
