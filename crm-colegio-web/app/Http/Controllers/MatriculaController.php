...
class MatriculaController extends Controller
{
    public function index(Request $request)
    {
        ...
        $alumnos    = Alumno::where('estado', 'activo')->orderBy('apellidos')->orderBy('nombres')->get();

        return view('enrollments.index', compact('matriculas', 'grados', 'secciones', 'alumnos'));
    }
    ...
    public function create(Request $request)
    {
        ...
        $grados   = Grado::with('secciones')->get();
        $secciones = Seccion::with('grado')->get();
        $alumnoSeleccionado = $request->get('alumno_id');
        return view('enrollments.create', compact('alumnos', 'grados', 'secciones', 'alumnoSeleccionado'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ((int) $seccion->grado_id !== (int) $request->grado_id) {
            return back()->withInput()->with('error', 'La seccion seleccionada no pertenece al grado elegido.');
        }

        $yaMatriculado = Matricula::where('alumno_id', $request->alumno_id)
            ->where('anio_escolar', $request->anio_escolar)
            ->where('estado', 'activo')
            ->exists();

        if ($yaMatriculado) {
            return back()->withInput()->with('error', 'El alumno ya tiene una matricula activa para ese año escolar.');
        }
