...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user->role === 'estudiante') {
            $alumno = $user->alumno()->with(['matriculas.grado', 'matriculas.seccion', 'pagos.concepto'])->first();
            ...
            $notas = $alumno
                ? Nota::with('materia')
                    ...
                    ->get()
                : collect();
            $promedio = $notas->whereNotNull('nota')->avg('nota');
            $mensajesNoLeidos = Mensaje::where('destinatario_id', $user->id)->where('leido', false)->count();

            return view('dashboard.estudiante', compact('alumno', 'matricula', 'notas', 'promedio', 'mensajesNoLeidos', 'anioActual'));
        }

        if ($user->role === 'docente') {
            $personal = $user->personal;
            $asignaciones = $personal
                ? Asignacion::with(['materia', 'seccion.grado'])
            ...
            return view('dashboard.docente', compact('personal', 'asignaciones', 'mensajesNoLeidos', 'anioActual'));
        }
