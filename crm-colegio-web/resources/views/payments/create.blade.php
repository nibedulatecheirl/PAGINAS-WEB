...
<div class="card">
    <div class="card-header">
        <span class="card-title"><i class="fas fa-money-bill-wave" style="color:#168a68;margin-right:8px;"></i>Datos del Pago</span>
    </div>
    <div class="card-body">
        ...
        <div class="grid grid-2">
            ...
            <div class="form-group">
                ...
                <input type="text" id="saldo_pago" class="form-control" value="S/. 0.00" readonly>
            </div>
        </div>
...
document.getElementById('concepto_select').addEventListener('change', function() {
    ...
    document.getElementById('monto_pagado').dataset.manual = '';
    actualizarSaldo();
});
function calcularPagado() {
    ...
    if (!pagadoEl.dataset.manual) {
        pagadoEl.value = Math.max(0, m - d).toFixed(2);
    }
    actualizarSaldo();
}
function actualizarSaldo() {
    ...
    const saldo = Math.max(0, m - d - p);
    document.getElementById('saldo_pago').value = 'S/. ' + saldo.toFixed(2);
    if (saldo > 0 && document.getElementById('estado_pago').value === 'pagado') {
        document.getElementById('estado_pago').value = 'pendiente';
    }
}
document.getElementById('monto').addEventListener('input', calcularPagado);
document.getElementById('monto_pagado').addEventListener('input', function(){ this.dataset.manual = '1'; actualizarSaldo(); });
document.getElementById('descuento').addEventListener('input', actualizarSaldo);
actualizarSaldo();
</script>
@endpush
