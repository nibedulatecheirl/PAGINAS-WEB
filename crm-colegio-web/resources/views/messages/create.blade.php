...
<div class="card">
    <div class="card-header">
        <span class="card-title"><i class="fas fa-envelope" style="color:#4f86bd;margin-right:8px;"></i>Nuevo Mensaje</span>
    </div>
    <div class="card-body">
        <div class="form-group">
            ...
            <select name="destinatario_id" class="form-control {{ $errors->has('destinatario_id') ? 'is-invalid':'' }}" required>
                ...
                @foreach($usuarios as $u)
                    <option value="{{ $u->id }}" {{ old('destinatario_id', request('reply'))==$u->id ? 'selected':'' }}>
                        {{ $u->name }} — {{ $u->role_label }}
                    </option>
                @endforeach
            </select>
