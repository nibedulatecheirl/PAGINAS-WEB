...
<div style="text-align:center;margin-bottom:20px;display:none;" class="print-header">
    <h2 style="font-size:20px;font-weight:800;">REPORTE DE DEUDAS PENDIENTES</h2>
    <p style="color:#66758a;font-size:13px;">Generado el {{ now()->format('d/m/Y H:i') }}</p>
</div>
                ...
                <tr>
                    ...
                    <td class="no-print">
                        <div style="display:flex;gap:6px;">
                            @if(auth()->user()->canAccess('alumnos'))
                            ...
                            @if(auth()->user()->canAccess('pagos'))
                                <a href="{{ route('pagos.index') }}?alumno_id={{ $alumno->id }}" class="btn btn-sm btn-primary btn-icon" title="Registrar pago"><i class="fas fa-money-bill"></i></a>
                            @endif
                        </div>
                    </td>
                </tr>
