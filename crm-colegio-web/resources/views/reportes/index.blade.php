...
<div class="grid grid-3" style="margin-bottom:24px;">
    ...
    <div class="card" style="padding:24px;text-align:center;cursor:pointer;transition:transform .2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform=''">
        <div style="width:60px;height:60px;border-radius:16px;background:linear-gradient(135deg,#065f46,#168a68);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:26px;color:white;">
    ...
    <div class="card" style="padding:24px;text-align:center;cursor:pointer;transition:transform .2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform=''">
        <div style="width:60px;height:60px;border-radius:16px;background:linear-gradient(135deg,#18324d,#4f86bd);display:flex;align-items:center;justify-content:center;font-size:26px;color:white;margin:0 auto 16px;">
    ...
    <div class="card" style="padding:24px;text-align:center;cursor:pointer;transition:transform .2s;" onmouseover="this.style.transform='translateY(-4px)'" onmouseout="this.style.transform=''">
        <div style="width:60px;height:60px;border-radius:16px;background:linear-gradient(135deg,#7f1d1d,#c2414b);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;font-size:26px;color:white;">
...
<div class="grid grid-2" style="margin-bottom:24px;">
    ...
    <div class="card">
        <div class="card-header">
            <span class="card-title"><i class="fas fa-chart-bar" style="color:#4f86bd;margin-right:8px;"></i>Ingresos Mensuales {{ $anio }}</span>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            <span class="card-title"><i class="fas fa-chart-pie" style="color:#168a68;margin-right:8px;"></i>Ingresos por Tipo de Concepto</span>
        </div>
...
<div class="card">
    <div class="card-header">
        <span class="card-title"><i class="fas fa-chart-bar" style="color:#6f669a;margin-right:8px;"></i>Distribución de Alumnos por Grado — {{ $anio }}</span>
    </div>
...
new Chart(document.getElementById('chartIngresosMes'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Ingresos (S/.)',
            data: ingArr,
            backgroundColor: ingArr.map((v,i) => i === new Date().getMonth() ? '#18324d' : 'rgba(79,134,189,0.68)'),
            borderRadius: 8,
            borderSkipped: false,
        }]
    },
...
new Chart(document.getElementById('chartTipo'), {
    ...
    data: {
        ...
        datasets: [{ data: tipoTotals.length ? tipoTotals : [1], backgroundColor: ['#4f86bd','#168a68','#c47a16','#6f669a','#c2414b'], borderWidth: 2, borderColor: '#fff' }]
    },
...
new Chart(document.getElementById('chartAlumnos'), {
    ...
    data: {
        ...
        datasets: [{ label: 'Alumnos', data: gradoTotals.length ? gradoTotals : [0], backgroundColor: 'rgba(111,102,154,0.74)', borderRadius: 6, borderSkipped: false }]
    },
    options: {
        indexAxis: 'y',
        responsive: true,
